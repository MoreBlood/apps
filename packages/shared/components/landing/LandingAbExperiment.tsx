...
export default function LandingAbExperiment({ app, children }: Props) {
	...
	const premiumPreferredRef = useRef(false)
	...
	const triggerInterrupt = useCallback(() => {
		if (triggeredRef.current || premiumPreferredRef.current || variant !== 'anti') return
		triggeredRef.current = true
		setPhase(reducedMotion ? 'minimal' : 'interrupt')
	}, [reducedMotion, variant])

	const dismissInterrupt = useCallback(() => {
		premiumPreferredRef.current = true
		triggeredRef.current = true
		setPhase('intro')
	...
	useEffect(() => {
		...
		const onScroll = () => {
			...
			if (y >= SCROLL_TRIGGER_PX && lastScrollY < SCROLL_TRIGGER_PX) triggerInterrupt()
			lastScrollY = y
		}
					...
					>
						<div className="landing-ab-interrupt__panel">
							...
							<div className="landing-ab-interrupt__actions">
								...
								<button type="button" className="landing-ab-interrupt__ghost" onClick={dismissInterrupt}>
