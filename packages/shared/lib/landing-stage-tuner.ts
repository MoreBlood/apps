...
export function formatLandingStageLayoutSnippet(
	...
	const deviceLines = slots
		.map((slot) => {
			...
			const mult = slot.scaleMult != null && slot.scaleMult !== 1 ? `, scaleMult: ${round(slot.scaleMult, 3)}` : ''
	...
	if (opts) {
		...
		if (opts.manualClusterScale && opts.scaleOverride != null) {
			lines.push('', `// Optional manual cluster scale: scaleOverride: ${round(opts.scaleOverride, 6)}`)
		}
	}
