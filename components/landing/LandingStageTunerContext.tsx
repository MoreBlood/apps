...
import {
	isLandingStageTuner,
	isLandingStageTunerFromEnv,
	landingStageId
} from '@/lib/landing-stage-tuner'
...
function mergeSlotsForLayout(
	layoutKey: LandingStageLayoutKey,
	override?: StageDeviceSlot[]
): StageDeviceSlot[] {
...
export function LandingStageTunerProvider({ children }: { children: ReactNode }) {
	...
	const open = useCallback(
		(next: LandingStageTunerActive, initial?: LandingStageTunerOverride) => {
	...
	const getOverride = useCallback(
		(stageId: string) => overrides[stageId],
		[overrides]
	)
	...
	const resetOverride = useCallback((stageId: string, layoutKey: LandingStageLayoutKey) => {
		...
		setActive((current) =>
			current?.stageId === stageId ? { ...current, layoutKey } : current
		)
...
function LandingStageTunerFab() {
	...
	const label = tuner.paneOpen && tuner.active
		? `Tuner: ${tuner.active.variant}`
		: 'Stage tuner — click a mockup'
